...
@interface CommodityViewController ()<UITableViewDelegate,UITableViewDataSource,UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout>
/* tableView */
...
- (void)setUpNav
{
    ...
    _topSearchView.frame = CGRectMake(50, 6, kScreen_Width - 110, 32);
    ...
    _voiceButton.frame = CGRectMake(_topSearchView.width - 40, 0, 35, _topSearchView.height);
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([_mainItem[_mainItem.count - 1].title isEqualToString:@"热门品牌"]) {
        if (indexPath.section == _mainItem.count - 1) {//品牌
            return CGSizeMake((kScreen_Width - tableViewH - 6 - DCMargin)/3, 60);
        }else{//商品
            return CGSizeMake((kScreen_Width - tableViewH - 6 - DCMargin)/3, (kScreen_Width - tableViewH - 6 - DCMargin)/3 + 20);
        }
    }else{
        return CGSizeMake((kScreen_Width - tableViewH - 6 - DCMargin)/3, (kScreen_Width - tableViewH - 6 - DCMargin)/3 + 20);
    }
