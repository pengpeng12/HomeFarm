...
//        [weakSelf.navigationController pushViewController:dcGMvC animated:YES];
    ...
    _topToolView.rightItemClickBlock = ^{
        NSLog(@"点击了首页分类");
        CommodityViewController *dcComVc = [CommodityViewController new];
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout *)collectionViewLayout sizeForItemAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {//9宫格组
        return CGSizeMake(kScreen_Width/5 , kScreen_Width/5 + DCMargin);
    }
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.section == 4) {
        if (indexPath.row == 0) {
            layoutAttributes.size = CGSizeMake(kScreen_Width, kScreen_Width * 0.38);
        }else if (indexPath.row == 1 || indexPath.row == 2 || indexPath.row == 3 || indexPath.row == 4){
            layoutAttributes.size = CGSizeMake(kScreen_Width * 0.5, kScreen_Width * 0.24);
        }else{
            layoutAttributes.size = CGSizeMake(kScreen_Width * 0.25, kScreen_Width * 0.35);
        }
    }
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForHeaderInSection:(NSInteger)section {
    ...
    if (section == 0) {
        return CGSizeMake(kScreen_Width, 230); //图片滚动的宽高
    }
    if (section == 2 || section == 4 || section == 5) {//猜你喜欢的宽高
        return CGSizeMake(kScreen_Width, 40);  //推荐适合的宽高
    }
    return CGSizeZero;
}
...
- (CGSize)collectionView:(UICollectionView *)collectionView layout:(UICollectionViewLayout*)collectionViewLayout referenceSizeForFooterInSection:(NSInteger)section {
    if (section == 0) {
        return CGSizeMake(kScreen_Width, 180);  //Top头条的宽高
    }
    if (section == 3) {
        return CGSizeMake(kScreen_Width, 80); // 滚动广告
    }
    if (section == 5) {
        return CGSizeMake(kScreen_Width, 40); // 结束
    }
    return CGSizeZero;
}
