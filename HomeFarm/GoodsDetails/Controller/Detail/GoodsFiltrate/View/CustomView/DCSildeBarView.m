...
- (DCFiltrateViewController *)filterView{
    if (!_filterView) {
        ...
        _filterView.view.frame = CGRectMake(kScreen_Width, 0, kScreen_Width * 0.8, kScreen_Height);
    }
    return _filterView;
}
...
+ (void)dc_showSildBarViewController{
    DCSildeBarView *obj = [[DCSildeBarView alloc]initWithFrame:CGRectMake(0, 0, kScreen_Width, kScreen_Height)];
...
- (void)addSubViews{
    ...
    [UIView animateWithDuration:AnimatorDuration animations:^{
        ...
        weakSelf.filterView.view.left = kScreen_Width * 0.2;
    }];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context
{
    if ([keyPath isEqualToString:@"frame"]) {
        ...
        if (x < kScreen_Width) {
            self.coverView.alpha = 0.4 * (1-x / kScreen_Width) + 0.1;
        }else{
...
- (void)panEvent:(UIPanGestureRecognizer *)recognizer{
    ...
    if(UIGestureRecognizerStateBegan == recognizer.state || UIGestureRecognizerStateChanged == recognizer.state){
        ...
        if (translation.x > 0 ) {//右滑
            self.filterView.view.left = kScreen_Width * 0.2 + translation.x;
        }else{//左滑
            
            if (self.filterView.view.left < kScreen_Width * 0.2) {
                self.filterView.view.left = self.filterView.view.left - translation.x;
            }else{
                self.filterView.view.left = kScreen_Width * 0.2;
            }
        }
    }else{
...
- (void)tapEvent{
    ...
    [UIView animateWithDuration:AnimatorDuration animations:^{
        ...
        weakSelf.filterView.view.left = kScreen_Width;
